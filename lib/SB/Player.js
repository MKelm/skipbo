...
SB.Player.prototype.cpuMoveCardToTargetStack = function(
  ...
  if (this.cpuLog) console.log("source card id", sourceCardId);
  if (typeof sourceCardId === "number") {
    var sourceCardValue = cardHandler.cards[sourceCardId].value;
    if (this.cpuLog) console.log("source card value", sourceCardValue);
    ...
    for (var i = 0; i < 4; i++) {
      ...
      if (sourceMatch === true) {
        ...
        if (spliceMode === true) {
          ...
          if (this.cpuLog) console.log("hand remove", sourceCardPos);
...
SB.Player.prototype.cpuTurn = function(game, playerId) {
  ...
  if (this.cpuLog) console.log("playerId", playerId);
  ...
  if (startListLength > 0) {
    ...
    if (this.cpuLog) console.log("start", match);
  }
  ...
  for (var i = 0; i < this.cardsHandMax; i++) {
    ...
    if (this.cpuLog) console.log("hand", tmpMatch);
  }
  ...
  for (var i = 0; i < 4; i++) {
    ...
    if (this.cpuLog) console.log("temp", tmpMatch);
  }
  ...
  if (match === true) {
          ...
          if (tempStack.list.length > 0) {
            ...
            if (tempCardId === "number") {
              ...
              if (tempCardValue + 1 == handCardValue) {
                ...
                tmpMatch = true;
                if (this.cpuLog) console.log("temp lower one", j);
              }
            }
          }
    ...
    // check random hand cards to empty or random temp stack
    if (tmpMatch === false) {
      ...
      if (this.cpuLog) console.log("hand card to temp", handCardPos, tempStackPos);
