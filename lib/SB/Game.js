...
SB.Game.prototype.initPlaceholderCards = function() {
  ...
  for (var i = offset; i < offset+8; i++) {
    this.cards[i] = { value : i, gfx : this.getCardGfx(i, true) };
  }
}
...
SB.Game.prototype.getCardGfx = function(value, placeholder) {
  ...
  $.extend(
    eventContent,
    { card: (placeholder === true) ? "placeholder" : "general", value: value }
  );
  var scope = this;
  var callback = function(mouse) {
    $.extend(eventContent, { mouse: mouse });
    scope.dispatchEvent( { type: "mousedown", content: eventContent } );
  };
...
SB.Game.prototype.handleEvent = function(scope, event) {
  if (event.type == "mousedown") {
    ...
    if ((position.x >= -250 * ratio || position.x <= 260 * ratio) &&
      ...
      this.clickPath = [{ type: "player0 hand card", card: card, value: value }];
      reset = false;
    } else if (position.x == 300 * ratio && position.y == 100 * ratio) {
      this.clickPath = [{ type: "player0 start card", card: card, value: value }];
      reset = false;
