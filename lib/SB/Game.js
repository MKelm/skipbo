...
SB.Game.prototype.showCards = function() {
  ...
  for (var i = 0; i < middlePositions.length; i++) {
    if (this.cardHandler.stacks["target"+i].visible !== null) {
      this.cardHandler.hideCard(this.cardHandler.stacks["target"+i].visible);
      this.cardHandler.stacks["target"+i].visible = null;
    }
    var listLength = this.cardHandler.stacks["target"+i].list.length;
    var cardId = (listLength > 0) ?
  ...
  for (i = 0; i < cardsPositions.length; i++) {
    if (this.cardHandler.stacks.players[0]["temp"+i].visible !== null) {
      this.cardHandler.hideCard(this.cardHandler.stacks.players[0]["temp"+i].visible);
      this.cardHandler.stacks.players[0]["temp"+i].visible = null;
    }
    var listLength = this.cardHandler.stacks.players[0]["temp"+i].list.length;
    var cardId = (listLength > 0) ?
      this.cardHandler.stacks.players[0]["temp"+i][listLength-1] :
  ...
  if (this.cardHandler.stacks.players[0].start.visible !== null) {
    this.cardHandler.hideCard(this.cardHandler.stacks.players[0].start.visible);
    this.cardHandler.stacks.players[0].start.visible = null;
  }
  var listLength = this.cardHandler.stacks.players[0].start.list.length;
  if (listLength > 0) {
    var cardId = this.cardHandler.stacks.players[0].start.list[listLength-1];
    this.cardHandler.moveCard(
      cardId,
  ...
  for (i = 0; i < cardsHandPositions.length; i++) {
    var cardId = this.cardHandler.stacks.players[0].hand.list[i];
