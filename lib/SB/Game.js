...
SB.Game.prototype.showCards = function() {
  ...
  for (i = 0; i < this.layout.targetStacks.length; i++) {
    ...
    if (cardId !== null) {
      this.removePlaceholderCardId(cardId);
  ...
  for (i = 0; i < this.layout.playerHandStack.length; i++) {
    cardId = this.cardHandler.stacks.players[0].hand.list[i];
...
SB.Game.prototype.handleEvent = function(scope, event) {
  if (event.type == "mousedown") {
    ...
    if ((position.x >= -250 * ratio || position.x <= 260 * ratio) &&
      ...
      // target start card
      this.clickPath = [{ type: "player0 start card", card: card, value: value, object: targetCardObj }];
      reset = false;
      ...
      if (this.clickPath.length == 1 &&
        ...
        // switch placeholder / hand card position, experimental state!!
        var tmpPos = targetCardObj.position;
        targetCardObj.position = this.clickPath[0].object.position;
        this.clickPath[0].object.position = tmpPos;
        this.clickPath.push({
          type: "player0 tmp card",
          card: card,
          value: value,
          object: targetCardObj,
          stackPosition: stackPosition
