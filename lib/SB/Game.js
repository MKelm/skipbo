...
SB.Game.prototype.initCards = function() {
  ...
  for (var i = 1; i <= this.cardsMax; i++) {
    for (var j = 1; j <= this.cardsMulti; j++) {
      this.cards[c] = { value : i, position : 0 };
      c++;
    }
  }
}
...
SB.Game.prototype.drawCard = function(x, y, value, open) {
  var width = 120 * sb.pixi.screen.ratio, height = 200 * sb.pixi.screen.ratio;
  var gfx = new PIXI.Graphics();
  var color = this.getCardColor(value, open);
  gfx.lineStyle(5, "0x"+color);
  gfx.beginFill("0xE6E6E6");
  gfx.moveTo(x - width / 2, y - height / 2);
  gfx.lineTo(x + width / 2, y - height / 2);
  gfx.lineTo(x + width / 2, y + height / 2);
  gfx.lineTo(x - width / 2, y + height / 2);
  gfx.lineTo(x - width / 2, y - height / 2);
  if (open === true) {
    var style = {font: Math.floor(48 * sb.pixi.screen.ratio) + "px " + "Arial", fill: color};
    var valueText = new PIXI.Text(value, style);
    valueText.anchor = { x: 0.5, y: 0.5 };
    valueText.position.x = x;
    valueText.position.y = y;
  } else {
    gfx.moveTo(x - width / 2, y + height / 2);
    gfx.lineTo(x + width / 2, y - height / 2);
  }
  ...
  if (open === true) {
    this.display.addChild(valueText);
  }
}
...
SB.Game.prototype.start = function() {
  ...
  this.drawCard(-60, 0, this.cards[71].value, true);
  this.drawCard(-180, 0, this.cards[70].value, true);
  this.drawCard(60, 0, this.cards[69].value, true);
  this.drawCard(180, 0, this.cards[68].value, true);
}
